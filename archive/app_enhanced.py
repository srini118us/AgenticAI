...
*Note: Configure OpenAI API key for detailed recommendations.*
    ...
    def _extract_single_system_findings_simple(self, documents: List[Any], system_id: str) -> tuple:
        """DYNAMIC: Extract findings for ONE specific system using intelligent content analysis without hardcoding"""
        critical_findings = []
        recommendations = []
        ...
        # Extract content specific to this system
        system_content = ""
        ...
        # DYNAMIC: Intelligent content analysis without hardcoding
        content_lower = system_content.lower()
        
        # 🔴 CRITICAL ISSUES - Dynamic detection based on actual content
        critical_indicators = [
            # Hardware and performance
            ('hardware.*resources.*exhausted', 'Hardware resources exhausted'),
            ('cpu.*utilization.*9[0-9]%', 'High CPU utilization (>90%)'),
            ('memory.*utilization.*9[0-9]%', 'High memory utilization (>90%)'),
            ('disk.*space.*exhausted', 'Disk space exhausted'),
            ('storage.*exhausted', 'Storage resources exhausted'),
            
            # Performance degradation
            ('severe.*performance.*problems', 'Severe performance problems detected'),
            ('critical.*performance.*issues', 'Critical performance issues detected'),
            ('response.*time.*exceeded', 'Response time exceeded thresholds'),
            ('throughput.*degraded', 'System throughput degraded'),
            
            # Security issues
            ('critical.*security.*issues?', 'Critical security issues detected'),
            ('security.*vulnerabilities', 'Security vulnerabilities found'),
            ('user.*system.*active.*valid', 'User SYSTEM account is active (security risk)'),
            ('password.*policy.*not.*enforced', 'Password policy not enforced'),
            ('audit.*disabled', 'Auditing is disabled'),
            
            # Configuration issues
            ('parameters.*deviate.*recommend', 'Parameters deviate from recommendations'),
            ('configuration.*issues', 'Configuration issues detected'),
            ('settings.*not.*optimal', 'Settings not optimal'),
            
            # Connection and service issues
            ('connection.*failed', 'Connection failures detected'),
            ('service.*failed', 'Service failures detected'),
            ('database.*connection.*issues', 'Database connection issues'),
            
            # General critical indicators
            ('red.*rating', 'Red rating detected'),
            ('critical.*rating', 'Critical rating detected'),
            ('severe.*problems.*may.*cause.*lose.*business', 'Severe problems that may cause business loss'),
            ('business.*impact.*critical', 'Critical business impact detected'),
            ('fatal.*error', 'Fatal errors detected'),
            ('severe.*error', 'Severe errors detected'),
        ]
        
        # 🟡 WARNING ISSUES - Dynamic detection based on actual content
        warning_indicators = [
            # Software and version issues
            ('outdated.*version', 'Outdated software versions detected'),
            ('support.*package.*months', 'Support packages are outdated'),
            ('patch.*level.*old', 'Patch level is outdated'),
            
            # Configuration warnings
            ('parameters.*deviate.*recommend', 'Parameters deviate from recommendations'),
            ('configuration.*suboptimal', 'Suboptimal configuration detected'),
            ('settings.*not.*optimal', 'Settings not optimal'),
            
            # Performance warnings
            ('performance.*degradation', 'Performance degradation detected'),
            ('slow.*response.*times', 'Slow response times detected'),
            ('resource.*utilization.*high', 'High resource utilization'),
            
            # Maintenance issues
            ('purge.*scheduling.*not.*conform', 'Purge job scheduling does not conform to recommendations'),
            ('backup.*issues', 'Backup issues detected'),
            ('maintenance.*window.*issues', 'Maintenance window issues'),
            
            # General warning indicators
            ('yellow.*rating', 'Yellow rating detected'),
            ('warning.*rating', 'Warning rating detected'),
            ('attention.*required', 'Attention required'),
            ('potential.*problem', 'Potential problems detected'),
        ]
        
        # Check for critical issues dynamically
        for pattern, description in critical_indicators:
            if re.search(pattern, content_lower):
                finding = f"🔴 [{system_id}] {description}"
                if finding not in critical_findings:
                    critical_findings.append(finding)
        
        # Check for warning issues dynamically
        for pattern, description in warning_indicators:
            if re.search(pattern, content_lower):
                finding = f"🟡 [{system_id}] {description}"
                if finding not in critical_findings:  # Add to critical_findings list (contains both)
                    critical_findings.append(finding)
        
        # 📋 RECOMMENDATIONS - Dynamic extraction based on actual content
        # Look for explicit recommendations
        if 'recommendation' in content_lower:
            sentences = re.split(r'[.!?]', system_content)
            for sentence in sentences:
        ...
        # Look for SAP Notes dynamically
        sap_note_matches = re.finditer(r'sap note (\d+)', content_lower)
        for match in sap_note_matches:
            note_num = match.group(1)
            rec = f"📋 [{system_id}] Refer to SAP Note {note_num}"
        ...
        # Look for specific action items dynamically
        action_patterns = [
            ...
            (r'secure.*network', 'Secure network settings'),
            (r'limit.*access', 'Limit critical access'),
        ]
    ...
    def _extract_multi_system_findings_simple(self, documents: List[Any]) -> tuple:
        """DYNAMIC: Extract findings for multiple systems using intelligent content analysis without hardcoding"""
        all_critical_findings = []
        all_recommendations = []
        ...
        # Find all systems mentioned dynamically
        systems_found = set()
        content_upper = all_content.upper()
        
        # Dynamic system detection patterns
        patterns = [
            r'\bSYSTEM\s+([A-Z0-9]{2,3})\b',
            r'\bSID\s+([A-Z0-9]{2,3})\b',
            r'\b([A-Z]{2,3})\s+SYSTEM\b',
        ...
        if not systems_found:
            # No systems found, return generic findings with dynamic patterns
            content_lower = all_content.lower()
            
            # Dynamic critical issue detection
            critical_patterns = [
        ...
        # Process each system with dynamic analysis
        for system_id in sorted(systems_found):
...
def main():
    ...
    # After document processing, show debug info
    if st.session_state.documents_processed:
        st.markdown('---')
        st.header('🛠️ Debug: System Extraction')
        # Get processed documents
        processed_docs = st.session_state.processing_results.get('processed_documents', [])
        all_text = ''
        for doc in processed_docs:
            if hasattr(doc, 'page_content'):
                ...
                all_text += str(doc) + '\n'
        # Show all detected system IDs
        import re
        system_patterns = [
            # Only valid SAP system IDs (2-3 letter codes)
            r'\bSYSTEM\s+([A-Z]{2,3})\b',
            r'\bSID\s*[:=]?\s*([A-Z]{2,3})\b',
            r'\[([A-Z]{2,3})\]',
            r'System ID:\s*([A-Z]{2,3})',
            r'System:\s*([A-Z]{2,3})',
            r'\b([A-Z]{2,3})\s+system\b',
            # SAP EWA specific patterns
            r'SAP System ID\s+([A-Z]{2,3})\b',
            r'System\s+([A-Z]{2,3})\s+Overview',
        ]
        found_ids = set()
        for pattern in system_patterns:
            ...
            for match in matches:
                if isinstance(match, tuple):
                    match = match[0]
                # Only accept 2-3 letter codes that are likely SAP system IDs
                if (len(match) in [2, 3] and 
                    match not in ['THE', 'AND', 'FOR', 'SAP', 'EWA', 'CPU', 'RAM', 'SID', 'SYSTEM', 'CMS', 'DB', 'BI', 'GB', 'MB', 'ABAP'] and
                    not match.isdigit()):  # Exclude pure numbers
                    found_ids.add(match.upper())
        st.info(f"🆔 Detected System IDs: {sorted(found_ids) if found_ids else 'None'}")
        
        # Show only relevant lines containing 'VMW' (filter out headers and noise)
        vmw_lines = []
        for line in all_text.split('\n'):
            line_upper = line.upper()
            if 'VMW' in line_upper:
                # Filter out header lines but KEEP hardware capacity content
                if not any(header in line_upper for header in [
                    'EARLYWATCH ALERT', 'PAGE', 'OVERVIEW', 'METRIC NAME', 'METRIC',
                    'RECOMMENDATION:', 'SOFTWARE CONFIGURATION'
                ]):
                    vmw_lines.append(line)
        
        # Also add any lines that mention hardware exhaustion (even if they don't contain VMW)
        hardware_lines = []
        for line in all_text.split('\n'):
        ...
        # Combine VMW lines with hardware lines
        all_relevant_lines = vmw_lines + hardware_lines
        
        st.write(f"**Relevant Lines containing 'VMW' ({len(vmw_lines)} found):**")
        for line in vmw_lines:
            st.code(line)
